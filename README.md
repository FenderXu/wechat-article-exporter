...
### 本地目录集成（OpenClaw / Obsidian）

如果你希望把文章直接同步到本地知识库目录，可以通过环境变量 `MP_EXPORT_ROOT` 指定导出根目录。

- 例如：`/Users/yourname/path/to/mpweixin`
- 目录下每个直接子目录会被视为一个公众号目录
- 已有文章的目录会自动从文章里的 `__biz` 信息识别公众号
- 空目录在登录有效时也会尝试按目录名自动识别公众号

这意味着你可以把现有的 Obsidian / OpenClaw 文章目录直接接到项目里，而不需要重新整理数据结构。
